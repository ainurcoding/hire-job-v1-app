...
export async function getStaticPaths() {
  const response = await axios({
    method: 'GET',
    url: `${process.env.APP_BACKEND_URL}/v1/user/list`
  })
  console.log(response.data.data.rows)
  const dataRes = await response.data.data.rows;
  // let dataRes = new Map([response.data.data.rows])
  // console.log(dataRes)
  const paths = dataRes.map((item) => {

    // const name = item.full_name
    const id = item.user_id
    // console.log(typeof id)
    return { params: { id: item.user_id } }
  })
  return {
    paths,
    fallback: true,
              ...
              <div className={`company-spec`}>
                <p className={`${gStyle["open_sans_lt"]} h6 text-muted`}>
                  {item.business_sector_recruiter ? (
                    ...
                    <>
                      data not found, please click on button edit, to complete this
                    </>
                  )}
                </p>
              </div>
              ...
              <div className="company-desc text-center">
                <p className={`${gStyle["open_sans_lt"]} h6 text-muted`}>
                  {item.personal_desc ? (
                    ...
                    <>
                      data description not found, please click on button edit, to complete this
                    </>
                  )}
                </p>
              </div>
